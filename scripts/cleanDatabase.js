// Database cleanup script using correct Firebase config
    ...
    for (const docSnapshot of snapshot.docs) {
      ...
      for (const [key, value] of Object.entries(data)) {
        if (typeof value === 'string' && value.includes('via.placeholder.com')) {
          ...
          foundUrls.push(`${docSnapshot.id}.${key}: ${value}`);
        ...
        if (Array.isArray(value)) {
          const updatedArray = value.map(item => {
            if (typeof item === 'string' && item.includes('via.placeholder.com')) {
              console.log(`   📍 Found old placeholder in ${docSnapshot.id}.${key}[]: ${item}`);
              foundUrls.push(`${docSnapshot.id}.${key}[]: ${item}`);
