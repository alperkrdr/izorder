...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        setUserEmail(null);
        console.log("Admin panel - no user logged in, redirecting to login");
  ...
  // Login sayfasındaysak normal render et
  if (pathname && pathname.includes('/login')) {
    return <>{children}</>;
  }
  ...
  const navigation = [
    ...
    { name: 'Veritabanı Temizlik', href: '/admin/database-cleanup', icon: FaBroom },
  ];
  ...
  return (
    <div className="h-screen flex overflow-hidden bg-gray-100">
      ...
      {/* Mobile sidebar */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-800 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out md:hidden`}>
